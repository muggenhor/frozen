...
#ifndef FROZEN_LETITGO_BITS_PIC_STRING_ARRAY_H
#define FROZEN_LETITGO_BITS_PIC_STRING_ARRAY_H
...
// A string_view that refers into a character array with an integer index.
// This has the advantage of being memory location independent, with regards to the location of the
// character data. As a result no relocations need to be generated. For position-independent-code
// this ensures this doesn't need relocations to happen when loading the executable or shared
// library containing instances of this class. For large data sets this can provide a significant
...
//
// Note that a string_view with something like Boost's `offset_ptr` would be able to achieve the
// same, in a much easier way. That class, however, relies on undefined behavior *. And even in that
// case: neither GCC nor Clang generate code free of the runtime relocation requirement, they
// generate the code themselves instead of letting the dynamic linker do it.
//
// *: boost::offset_ptr triggers undefined behavior when it subtracts two pointers not part of the
//    same array and again when it subsequently adds the integer result of that to a pointer, moving
...
constexpr T make_absolute(const ext_string<CharT>& self, const CharT* base) noexcept {
...
struct pic_iterator {
  ...
  using reference               = std::conditional_t<
    IsConst,
    pic_reference_type<const value_type&>,
    pic_reference_type<value_type&>
  >;
  ...
  constexpr pic_iterator(const element_t<value_type>* string_data, storage_type* pos) noexcept
    : string_data_{string_data}
    , pos_{pos}
  {}

  constexpr reference operator*() const noexcept {
    return make_absolute<reference>(*pos_, string_data_);
  }
  ...
  friend constexpr bool operator==(const pic_iterator& lhs, const pic_iterator& rhs) noexcept {
    constexpr_assert(lhs.string_data_ == rhs.string_data_, "compared iterators must belong to the same range");
    return lhs.pos_ == rhs.pos_;
  }
...
private:
  const element_t<value_type>* string_data_ = nullptr;
  storage_type* pos_ = nullptr;
};

template <typename T, std::size_t N, std::size_t string_size>
struct pic_string_array {
  using iterator                = pic_iterator<T, false>;
  using const_iterator          = pic_iterator<T, true>;
...
private:
  ...
  element_t<value_type> string_data_[string_size] = {};

  struct appender;

  template <typename Iter, std::size_t... I, std::size_t... J>
  constexpr pic_string_array(Iter iter, std::index_sequence<I...>, std::index_sequence<J...>, appender append = {})
    : data_{((void)I, append(*iter++))...}
    , string_data_{append.string_data[J]...}
  {
  }
...
public:
  ...
  constexpr pic_string_array(const value_type (&... values)[Ns]) 
    : pic_string_array({ value_type{values, Ns > 0 ? Ns - 1 : Ns}... })
  {
  }

  // Constructor intended for converting between different variations of const
  template <typename U, std::enable_if_t<std::is_convertible<U, T>::value, std::size_t> M, std::size_t other_string_size>
  constexpr pic_string_array(const pic_string_array<U, M, other_string_size>& rhs)
    : pic_string_array(rhs.begin(), std::make_index_sequence<N>(), std::make_index_sequence<string_size>()) {}

  constexpr pic_string_array(std::initializer_list<value_type> items)
    : pic_string_array(items.begin(), std::make_index_sequence<N>(), std::make_index_sequence<string_size>()) {}

  // Iterators
  constexpr       iterator begin()       noexcept { return {string_data_, std::begin(data_)}; }
  constexpr const_iterator begin() const noexcept { return {string_data_, std::begin(data_)}; }
  constexpr       iterator end()         noexcept { return {string_data_, std::end(data_)  }; }
  constexpr const_iterator end()   const noexcept { return {string_data_, std::end(data_)  }; }
...
private:
  ...
  struct appender {
    ...
    element_t<value_type> string_data[string_size] = {};

    template <typename StringView>
    constexpr auto operator()(const StringView& item)
      -> std::enable_if_t<
        std::is_same<element_t<StringView>, element_t<value_type>>::value
      , ext_string<typename StringView::value_type>
      >
    {
      const bool is_nul_terminated = !item.empty() && item.back() == '\0';

      pic_store_type<StringView> str_value{store_pos, item.size()};

      // constexpr std::copy before C++20
      for (const auto& val : item) {
        string_data[store_pos++] = val;
      }
