...
template <class Key, std::size_t N, class Compare = std::less<Key>,
          class Container = bits::carray<Key, N>> class set {
...
class set<Key, 0, Compare, Container> {
...
constexpr auto make_set(
  ...
  constexpr const auto string_size = bits::accumulate({Ns...});
  using container_type = bits::pic_string_array<T, sizeof...(Ns), string_size>;
