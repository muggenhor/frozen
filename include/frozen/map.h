...
#include "frozen/bits/pic_string_array.h"
...
public:
  ...
  constexpr map(container_type items, Compare const &compare)
      : less_than_{compare}
      , items_{bits::quicksort(bits::remove_cv_t<container_type>{items}, less_than_)} {}
...
class map<Key, Value, 0, Compare, Container> {
  using container_type = Container;
  [[no_unique_address]] impl::CompareKey<Compare> less_than_;
...
constexpr auto make_map(
  ...
  , U> const&... items) {
  constexpr const auto key_string_size = bits::accumulate({KNs...});
  using container_type = bits::pic_string_array<std::pair<const T, U>, sizeof...(KNs), key_string_size>;
...
constexpr auto make_map(
  ...
  constexpr const auto key_string_size = bits::accumulate({VNs...});
  using container_type = bits::pic_string_array<std::pair<const T, U>, sizeof...(VNs), key_string_size>;
...
constexpr auto make_map(
  ...
  , bits::element_t<U> const (&)[VNs]> const&... items) {
  constexpr const auto key_string_size = bits::accumulate({KNs...});
  constexpr const auto val_string_size = bits::accumulate({VNs...});
  using container_type = bits::pic_string_array<std::pair<const T, U>, sizeof...(KNs), key_string_size + val_string_size>;
  return map<T, U, sizeof...(KNs), Compare, container_type>{container_type{items...}, compare};
}
